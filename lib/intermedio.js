 ...
 *  `customAnte`, si se manda, reemplaza el ante por defecto para esta ronda (lo define el admin). */
...
function dealTurnCards(state) {
  ...
  // Ni las parejas (3 y 3) ni las consecutivas (no hay número en medio) se
  // pueden jugar — el turno se salta solo en ambos casos.
  state.forcedSkip = isPair || isConsecutive;
  state.phase = "turn";
  state.isPair = isPair;

  const reasonTxt = isPair ? " (PAR — no se puede apostar)" : isConsecutive ? " (CONSECUTIVAS — sin apuesta posible)" : "";
  pushLog(state, `Turno de jugador — cartas: ${a.rank}${a.suit} y ${b.rank}${b.suit}${reasonTxt}.`);
}
...
function placeBet(state, players, playerId, amount) {
  ...
  if (state.forcedSkip) return { ok: false, error: "Este turno no se puede jugar (pareja o cartas consecutivas) — se salta automáticamente." };
...
function skipTurn(state) {
  ...
  pushLog(state, state.forcedSkip ? "Turno saltado automáticamente (pareja o cartas consecutivas)." : "El jugador decidió no apostar este turno.");
  advanceTurn(state);
  return { ok: true };
}
...
/** Revela la tercera carta y liquida el turno: gana si queda estrictamente en medio, pierde si no
 *  (incluyendo si empata exactamente con uno de los límites — sin penalización doble). */
function revealThird(state, players) {
  ...
  if (third.value > lo && third.value < hi) {
    ...
    // Pierde tanto si queda fuera del rango como si empata exactamente con
    // alguno de los dos límites — siempre se cobra el monto apostado, sin
    // penalización extra.
    outcome = "lose";
    delta = -amount;
    potDelta = amount;
  ...
  pushLog(
    ...
    `Tercera carta: ${third.rank}${third.suit} → ${
      outcome === "win" ? `¡Ganó $${delta}!` : `Perdió $${Math.abs(delta)} hacia el pozo.`
    }`
  );
